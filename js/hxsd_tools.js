...
//Ajax请求数据的函数------------------------------------------------------------------------------------------------
  ajax:function (get,text,file){
	var xhr=new XMLHttpRequest();     //点击的时候创建构造函数XMLHttpRequest()的对象；
	xhr.open(get,text,true)  //给对象的open方法传入参数；
	xhr.onreadystatechange=function(){  //给对象绑定事件(也就是对象的状态信息改变时，执行行为)
		if(xhr.readyState==4 && xhr.status==200) {  //判断对象的状态信息是不是4（数据是否传回）如果传回则执行里面的行为；{对象的状态信息有5种，上网查}；
			file(xhr.responseText)//以参数的形式回调行为函数，并给回调的函数传入参数
		}
		else{
			file(xhr.status)//以参数的形式回调行为函数，并给回调的函数传入参数
		}
	}
	xhr.send(null);//向服务器发出请求；发出请求之后——服务器应答--（应答无法响应）——浏览器输出文件
}
}